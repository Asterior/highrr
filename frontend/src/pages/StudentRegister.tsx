...
const StudentRegister = () => {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      login(user);
      toast({ title: "Student account created", description: "Complete your profile and start applying." });
      navigate("/candidate", { replace: true });
    } catch (error: any) {
      toast({ title: "Registration failed", description: error.message || "Could not create student account", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-violet-50/40 to-cyan-50/30 flex items-center justify-center p-6">
      <div className="w-full max-w-md bg-card border border-border rounded-2xl p-6 shadow-card">
        <h1 className="text-2xl font-bold text-foreground">Register As Student</h1>
        <p className="text-sm text-muted-foreground mt-1">Create your student account to build profile, upload resume, and apply.</p>
